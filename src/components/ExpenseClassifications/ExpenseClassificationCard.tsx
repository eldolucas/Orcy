...
interface ExpenseClassificationCardProps {
  classification: ExpenseClassification;
  onEdit: (classification: ExpenseClassification) => void;
  onDelete: (id: string) => void;
}

export function ExpenseClassificationCard({ 
  classification, 
  onEdit, 
  onDelete 
}: ExpenseClassificationCardProps) {
          ...
          <div className="flex-1">
            <div className="flex items-center space-x-3">
              ...
              <span className={`px-2 py-1 text-xs font-medium rounded-full ${getTypeColor(classification.type)}`}>
                {expenseTypeLabels[classification.type as keyof typeof expenseTypeLabels]}
              </span>
            </div>
